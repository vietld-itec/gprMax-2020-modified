...
def norm(_inp):
    ...
    for i in range(len(_inp)):
        ...
        norm.append(tmp_Val)
    return norm
def mpl_plot(filename, outputdata, dt, dx, er, gmin, gmax, start, rxnumber, rxcomponent, isnorm = False):
    ...
    plt.title('Simulated B-scan',style='normal', fontsize=18)
...
if __name__ == "__main__":
    ...
    for rx in range(1, nrx + 1):
        outputdata, dt, dx, start = get_output_all_data(args.outputfile, rx, 'Ez')
        #print("dt=%f, dx=%f, start=%f",dt,dx,start)
        #print("dt={},dx={}, len[output]={}, {}".format(dt,dx,len(outputdata),len(outputdata[0])))
        #Convert to depth

        plthandle = mpl_plot(args.outputfile, outputdata, dt, dx, args.er, args.gmin,args.gmax, start, rx, 'Ez',isnorm = args.norm)
